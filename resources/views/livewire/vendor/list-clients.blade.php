<div class="flex flex-col gap-6 p-4 md:p-6">
    <x-auth-header
        ...
        :description="__('Consulte, filtre e gerencie os dados dos clientes.')"
    />
    ...
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg p-4">
        <div class="flex flex-col md:flex-row items-center justify-between space-y-3 md:space-y-0 md:space-x-4 mb-4">
            <div class="w-full md:w-1/2">
                <label for="client-search" class="sr-only">{{ __('Buscar') }}</label>
                <div class="relative">
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                    @forelse ($clients as $client)
                        <tr wire:key="client-{{ $client->id }}" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs font-medium rounded-full {{ $client->status === App\Enums\UserStatus::Active ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300' }}">
                                    {{ $client->status->label() }}
                                </span>
                            </td>
                        </tr>
